...
if     length(P1C1_filename) == 8 &&  length(P1P2_filename) == 8
        %====== Download via built-in websave (portable: no curl.exe needed)
        websave(P1C1_filename, ['ftp://ftp.aiub.unibe.ch/CODE/' P1C1_filename]);
        websave(P1P2_filename, ['ftp://ftp.aiub.unibe.ch/CODE/' P1P2_filename]);
...
elseif (exist(P1C1_filename, 'file') && exist(P1P2_filename, 'file'))
       %====== Read Satellites DCB
        ...
        websave([P1C1_filename '.Z'], ['ftp://ftp.aiub.unibe.ch/CODE/' year '/' P1C1_filename '.Z']);
        websave([P1P2_filename '.Z'], ['ftp://ftp.aiub.unibe.ch/CODE/' year '/' P1P2_filename '.Z']);
        gunzip([P1P2_filename '.Z']);
        gunzip([P1C1_filename '.Z']);
        %====== Read Satellites DCB
        P1C1Row_fileDCB = ReadRowDCB(P1C1_filename);
        P1P2Row_fileDCB = ReadRowDCB(P1P2_filename);
        P1C1_DCB = ReadDCB(P1C1_filename,P1C1Row_fileDCB-2);
